...
Model::Node Model::AdjustCoordinates(double lat, double lon)
{    
    ...
    else if (lat > m_MaxLat || lon > m_MaxLon || lat < m_MinLat || lon < m_MinLon)
    {
        std::cout << "input point " << lat
                  << "(lat), " << lon
                  << "(lon) is out of bounds --excluded";
    }
    else 
    {  
        input_node.x = (lon2xm(lon) - min_x) / m_MetricScale;
        input_node.y = (lat2ym(lat) - min_y) / m_MetricScale; 
